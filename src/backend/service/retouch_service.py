...
from util.value_mapper import map_contrast_value_to_pillow_value, map_color_value_to_pillow_value, \
    map_brightness_value_to_pillow_value
    ...
    if retouch_type == 'brightness':
        if not validate_new_value_for_retouch_type(retouch_type=retouch_type, new_value=new_value):
            error_message = f'Invalid {retouch_type} value: {new_value}\n' \
                            f'Valid range: [{SLIDER_MIN_VALUE}, {SLIDER_MAX_VALUE}]'
            return get_error_image_file(error_message)
        enhancer = ImageEnhance.Brightness(pillow_img)
        mapped_value = map_brightness_value_to_pillow_value(new_value)
        ...
        mapped_value = map_color_value_to_pillow_value(new_value)
