...
def delete_temp_files(suffix: str) -> None:
    ...
    file_list = glob.glob(
        f'{temp_dir}{os.sep}IMAGE_MANIPULATOR*{suffix}', recursive=True)
...
def clean_retouch_output_for_user(user_name: str) -> None:
    ...
    output_files = glob.glob(
        f'{path}{os.sep}RETOUCH_{user_name}*', recursive=True)
    for file in output_files:
        os.remove(file)


def clean_back_remove_output_for_user(user_name: str) -> None:
    path = os.path.join(os.getcwd(), 'resources', 'output')
    output_files = glob.glob(
        f'{path}{os.sep}BACK_REMOVE_{user_name}*', recursive=True)
    for file in output_files:
        os.remove(file)
